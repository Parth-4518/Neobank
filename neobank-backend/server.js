...
// Send Add Money Email
app.post('/api/send-add-money-email', async (req, res) => {
  ...
  try {
    const mailOptions = {
      from: `NeoBank <${process.env.EMAIL_USER}>`,
      to: email,
      subject: 'Money Added to Your NeoBank Wallet - Confirmation',
      html: `
          ...
          <div style="background: white; padding: 30px; border-radius: 0 0 10px 10px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
            ...
            <div style="background: #f0f9ff; padding: 20px; border-radius: 8px; margin: 20px 0;">
              ...
              <div style="display: flex; justify-content: space-between;">
                ...
                <span style="color: #1e3a8a;">${new Date().toLocaleString()}</span>
              </div>
            </div>
...
app.listen(PORT, () => {
  console.log(`NeoBank Email Server on port ${PORT}`);
});
